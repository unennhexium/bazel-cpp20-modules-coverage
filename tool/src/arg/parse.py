...
from src.arg.range import RangeAction
from src.log.const import Color
...
{CB}DESCRIPTION{CR}
    ...
    because of not satisfied {CM}#ifdef{CR}/{CM}#ifndef{CR} pragmas around the marker.
        ...
        epilog=f"""
{CB}ENVIRONMENT{CR}
    {CU}LOG_LEVEL{CR} setting this takes precedence over {CG}-l{CR},{CC}--log{CR} option; see option
    description for the list of possible values; set but empty equal setting 'DEBUG'.

    {CU}COLOR{CR} setting this takes precedence over {CG}-c{CR},{CC}--color{CR} option; see option
    descripton for the list of possible values; set but empt equal setting 'never'.
    Note that setting this or option does not affect the colorization of the help
    text. It is fixed to 'auto'.
