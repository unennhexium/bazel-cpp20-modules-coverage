...
from src.arg.type import Log, Paths, Stages, Width
from src.lib.gen import LineFilter, dummy
from src.lib.util import is_list_wo_none
from src.stage.mid import mid
from src.stage.post import post
from src.stage.pre import pre
...
class Arguments:
    ...
    def _stages(self) -> tuple[LineFilter, LineFilter, LineFilter]:
        args = self.raw
        full = "all" in args.stage
        nnone = "none" not in args.stage
        return (  # type: ignore[assignment]
            pre if (full or "pre" in args.stage) and nnone else dummy,
            mid if (full or "mid" in args.stage) and nnone else dummy,
            post if (full or "post" in args.stage) and nnone else dummy,
        )
    ...
    def _log_level(self, levels: dict[str, int]) -> int:
        args = self.raw
        if (ll := env.get("LOG_LEVEL", None)) is not None:
            if len(ll) == 0:
    ...
    def _log_color(self) -> bool:
        args = self.raw
        if (cl := env.get("COLOR", None)) is not None:
            if len(cl) == 0:
                return False
            match cl:
                case "auto":
